...
for (yearmonth_ind in yearmonth_inds){
  ...
  for (dayofmonth_ind in dayofmonth_inds){
        ...
        query <-  paste('select distinct "TailNum" from FLIGHT_DELAY.FLIGHTS_UTC_CORRECT where  
                    "Year"=\'',select_year,'\'  and "Month"=\'',select_month,'\' and "DayofMonth"=\'',select_dayofmonth,'\' group by "TailNum"',sep='');
        TailNum<- dbGetQuery(conn,query)   
        TailNum<-na.omit(TailNum)
        tailnum_inds<-1:nrow(TailNum)
        tailnum_ind<-1
     for (tailnum_ind in tailnum_inds){
        ...
        if (nrow(select_airports)>0){
          ...
          newline<-tailnum_delay
         }
    }
  }
  name <-  paste('tailnum_delay_',select_year,'_',select_month,'.csv',sep='');
