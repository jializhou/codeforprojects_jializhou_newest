...
for (yearmonth_ind in yearmonth_inds){
  ...
  tailnum_inds2<-length(tailnum_inds1):nrow(TailNum)
  for (tailnum_ind in tailnum_inds2){
    for (dayofmonth_ind in dayofmonth_inds){
      select_dayofmonth<-DayofMonth[dayofmonth_ind,1]
      select_tailnum<-TailNum[tailnum_ind,1]
      query<- paste('select count(*) as cnt,"OriginAirportID" as "OriginAirport", "DestAirportID" as "DestAirport", "OriginCityName" as "OriginCity","DestCityName" as "DestCity",
                    "CRSDEPDATETIME" as "DEPDATETIME","CRSARRDATETIME" as "ARRDATETIME","DepDelay" as "DepDelay",
                    "ArrDelay" as "ArrDelay"from FLIGHT_DELAY.FLIGHTS_UTC_CORRECT where "DepDelay" >0 or "ArrDelay" > 0 and
      ...
      if (nrow(select_airports)>0){
        ...
        tailnum_delay <- rbind(tailnum_delay,newline);       
      }
    }
  }
  name <-  paste('tailnum_delay2_',select_year,'_',select_month,'.csv',sep='');
  write.csv(tailnum_delay,name)
}


tailnum_delay<- tailnum_delay[-1,]
